  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const queryPageNo = 0; const isPushListConsistent = false
...
function dealNEPResult(nepResult: any, subject: AniSub): number {
  ...
  try {
    ...
    const current_episode = subject.getAnimeInstance().current_episode; const startEpiNum = subject.getAnimeInstance().eps!
